...
/**
 * Hook pour les animations d'entrée/sortie optimisées
 */
export function useNavigationAnimations() {
  const getStaggerDelay = useCallback(
    (index: number, baseDelay = 0.1, increment = 0.05) => {
  ...
  const getSpringConfig = useCallback(
    (type: 'smooth' | 'snappy' | 'bouncy' = 'smooth') => {
      const configs = {
        smooth: {
          type: 'spring',
          stiffness: 100,
  ...
  const getTapAnimation = useCallback(
    () => ({
      scale: 0.95,
      transition: {
        type: 'spring',
